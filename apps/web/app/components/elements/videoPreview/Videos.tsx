...
import { useSizeImage } from "@/app/hooks/useSizeImage";
...
function VideoPreview({ videos, title, btnUrl, className }: VideoPreviewProps) {
  // Фильтруем только трендовые видео  
  // Если нет трендовых видео, не показываем компонент
...
// Компонент карточки для трендового видео
function TrendingVideoCard({ video }: { video: VideoItem }) {
