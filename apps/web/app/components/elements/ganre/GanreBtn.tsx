...
import { useGanreBtn } from "@/app/stores/modal";
import { genres } from "@/app/constants/FakeGanre";
                ...
                {isOpen &&
                    <motion.ul
                        ...
                        className="absolute flex flex-wrap items-start gap-3 text-md px-10 py-5 w-250 
                                    bg-linear-to-br from-white/30 via-purple-900/40 to-pink-900/30 
                                    backdrop-blur-[3px] border border-white/20 
                                    shadow-2xl shadow-purple-900/30 rounded-2xl text-white z-30 top-15 right-0">
                        {genres.map((genre, index) => {
                            return (
                                <motion.li className="" key={`${index}-${genre}`}>
                                    <Link
                                        key={genre.slugName}
